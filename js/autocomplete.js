...
function seeSites(relPath,listName) {
	...
	window.location = relPath + filename+op+"&sites="+site_list.toString(); 	
	return false;
}
...
function autoComplete (field, selectName, property, forcematch) {
...
function putOption(text,value,to) {
	var options = new Object();
	if (hasOptions(to)) {
		for (var i=0; i<to.options.length; i++) {
			options[to.options[i].value] = to.options[i].text;
	...
	to.options[index] = new Option( text, value, false, false);
