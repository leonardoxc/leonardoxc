...
//
// $Id: EXT_thermals.php,v 1.3 2010/03/14 20:56:10 manolis Exp $                                                                 
//
//************************************************************************
...
$sql .= "COS(longitude /57.2958 - " . $lon . "/57.2958)))) ";
...
//		echo "<search>";
		...
		while ($row = mysql_fetch_assoc($dbres)) { 
			if ($i>0)$res.=" ,\n";
			$res.=' { "lat":'.$row['latitude'].', "lon":'.$row['longitude'].' , "d":"'.$row['climbseconds'].'", "m":"'.$row['climbmeters'].'",  "c":"'.$row['class'].'" } ';
			//print_r($row);
