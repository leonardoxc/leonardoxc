...
//
// $Id: CL_convertTrack.php,v 1.2 2008/11/29 22:46:06 manolis Exp $                                                                 
//
//************************************************************************
...
function gpx2igc($gpxFile, $igcFile) 
{
	$l=file($gpxFile);
	for($i=0;$i<count($l);$i++) {
		$l[$i]=trim($l[$i]);
	...
	//echo $lines;
	//exit;
	//<trk><name>the track</name><trkseg>
	
	
	if( ! preg_match("/<trkseg>(.*)</trkseg>/i",$lines,$matches) ) {
		return 0;
	}
	...
	$trkseg=explode(" ",trim($matches[1]));

	//<trkpt lat="28.0153298404082" lon="86.8629527172877"><ele>5455</ele></trkpt>
	if( ! preg_match("/<trkpt *lat=\"(.*)\" *lon=\"(.*)\"><ele>(.*)<\/ele> *<\/trkpt>/i",$trkseg,$matches) ) {
		return 0;
	}
